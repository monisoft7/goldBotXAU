"""Read-only MT5 XAUUSD M15 CSV exporter."""
...
def export_xauusd_m15_csv(
    *,
    ...
    try:
        ...
        start = _date_start_utc(from_date)
        end = _date_end_utc(to_date)
        rates = mt5.copy_rates_range(normalized_symbol, mt5.TIMEFRAME_M15, start, end)
        ...
        if not rows:
            return _report(
                ...
                errors=["No M15 rates returned for requested range."],
            )

        output_path = _output_path(data_dir, normalized_symbol, from_date, to_date)
...
def _output_path(data_dir: str | Path, symbol: str, from_date: str, to_date: str) -> Path:
    clean_symbol = re.sub(r"[^a-z0-9]+", "", symbol.lower())
    return Path(data_dir) / f"xauusd_m15_{clean_symbol}_{from_date}_{to_date}.csv"
