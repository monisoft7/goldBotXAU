...
from src.research.xauusd_external_shortlist_train_validation_board import (
    ...
    save_xauusd_external_shortlist_train_validation_board,
)


def main() -> int:
    parser = argparse.ArgumentParser(description="Run v0_53 XAUUSD external shortlist train/validation board.")
    ...
    save_xauusd_external_shortlist_train_validation_board(report, args.output)

    text = json.dumps(report, indent=2, sort_keys=True)
    if args.as_json:
        print(text)
    else:
        print(f"xauusd_external_shortlist_board_status={report['board_status']}")
    return 0
