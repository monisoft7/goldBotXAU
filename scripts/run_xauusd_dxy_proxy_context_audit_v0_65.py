...
from src.research.xauusd_dxy_proxy_context_audit import (
    ...
    save_xauusd_dxy_proxy_context_audit,
)


def main() -> int:
    parser = argparse.ArgumentParser(description="Run v0_65 XAUUSD DXY/USD proxy context feasibility audit.")
    ...
    save_xauusd_dxy_proxy_context_audit(report, args.output)

    text = json.dumps(report, indent=2, sort_keys=True)
    if args.as_json:
        print(text)
    else:
        print(f"xauusd_dxy_proxy_context_audit_status={report['audit_status']}")
    return 0
