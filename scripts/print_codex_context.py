...
def _paper_forward_watcher_summary(root: Path) -> dict[str, Any] | None:
    watcher_path = _report_path(root, "xauusd_paper_forward_watcher_v0_87.json")
    ...
    ) and report.get("candidate_rules_preserved") is True
    return {
        ...
        "data_source_status": report.get("data_source_status"),
        "real_market_observation_started": report.get("real_market_observation_started"),
        "paper_observation_only": report.get("paper_observation_only"),
        "source_file_count": len(report.get("source_files_used", []))
        if isinstance(report.get("source_files_used"), list)
        else None,
        "timeframes_used": report.get("timeframes_used"),
        "watch_record_count": report.get("watch_record_count"),
        "recommended_next_step": report.get("recommended_next_step") or report.get("next_recommended_step"),
        "safety_locked": safety_locked,
    }


def _paper_forward_watcher_loop_summary(root: Path) -> dict[str, Any] | None:
    loop_path = _report_path(root, "xauusd_paper_forward_watcher_loop_v0_88.json")
...
def _paper_forward_outcome_tracker_summary(root: Path) -> dict[str, Any] | None:
    tracker_path = _report_path(root, "xauusd_paper_forward_outcome_tracker_v0_89.json")
    if not tracker_path.exists():
        return None
    report = json.loads(tracker_path.read_text(encoding="utf-8"))
    safety_locked = all(
        report.get(key) is False
        for key in (
            "execution_allowed",
    ...
    return {
        "tracker_version": report.get("tracker_version"),
        "tracker_status": report.get("tracker_status"),
        "source_loop_version": report.get("source_loop_version"),
        "records_read": report.get("records_read"),
...
def _oil_conditioned_event_study_summary(root: Path) -> dict[str, Any] | None:
    study_path = _report_path(root, "xauusd_oil_conditioned_event_study_v0_72.json")
    if not study_path.exists():
        return None
    report = json.loads(study_path.read_text(encoding="utf-8"))
    return {
        ...
        "study_status": report.get("study_status"),
        "selected_proxy_symbol": report.get("selected_proxy_symbol"),
        "fallback_proxy_symbol": report.get("fallback_proxy_symbol"),
        "event_count": report.get("event_count"),
        "clear_lead_count": report.get("clear_lead_count"),
        "train_validation_only": report.get("train_validation_only"),
        "oos_used": report.get("oos_used"),
...
def _external_yield_sample_validator_summary(root: Path) -> dict[str, Any] | None:
    validator_path = _report_path(root, "xauusd_external_yield_sample_validator_v0_75.json")
...
def _external_yield_asof_alignment_design_summary(root: Path) -> dict[str, Any] | None:
    ...
    return {
        "v": report.get("alignment_version"),
        "status": report.get("alignment_status"),
        "n": [
            report.get("records_seen"),
            report.get("records_alignable"),
            report.get("records_not_alignable"),
            report.get("target_timestamp_count"),
            report.get("aligned_target_count"),
            report.get("unaligned_target_count"),
...
def _external_yield_label_design_summary(root: Path) -> dict[str, Any] | None:
    label_path = _report_path(root, "xauusd_external_yield_label_design_v0_78.json")
    if not label_path.exists():
        return None
    report = json.loads(label_path.read_text(encoding="utf-8"))
    safety_locked = (
        all(
            report.get(key) is False
            for key in (
    ...
    return {
        "v": report.get("label_design_version"),
        "status": report.get("label_design_status"),
        "count": report.get("label_count"),
        "next": report.get("recommended_next_step"),
        "safe": safety_locked,
    }


def _external_yield_label_fixture_application_summary(root: Path) -> dict[str, Any] | None:
    application_path = _report_path(root, "xauusd_external_yield_label_fixture_application_v0_79.json")
    if not application_path.exists():
        return None
    report = json.loads(application_path.read_text(encoding="utf-8"))
    safety_locked = (
        all(
            report.get(key) is False
            for key in (
        ...
        and report.get("no_lookahead_policy_confirmed") is True
        and report.get("backward_asof_only") is True
        and report.get("synthetic_thresholds_used") is True
    )
    return {
        "v": report.get("application_version"),
        "n": [len(report.get("labels_requested", [])), len(report.get("labels_applied", [])), len(report.get("labels_not_applicable", [])), report.get("fixture_record_count"), report.get("synthetic_target_timestamp_count")],
        "safe": safety_locked,
    }
...
def _master_trading_path_reentry_board_summary(root: Path) -> bool | None:
    board_path = _report_path(root, "xauusd_master_trading_path_reentry_board_v0_81.json")
    if not board_path.exists():
        return None
    report = json.loads(board_path.read_text(encoding="utf-8"))
    safety_locked = (
        ...
        and report.get("train_validation_only") is True
        and report.get("oos_allowed_now") is False
        and report.get("demo_allowed_now") is False
        and report.get("live_allowed_now") is False
    )
...
def _trading_decision_sprint_summary(root: Path) -> list[Any] | None:
    sprint_path = _report_path(root, "xauusd_trading_decision_sprint_v0_84.json")
    ...
    safety_locked = (
        ...
        and report.get("train_validation_only") is True
        and report.get("validation_used_for_rescue_selection") is False
        and report.get("rescue_variant_evaluated_count", 0) <= 1
    )
    return [
        report.get("sprint_version"),
        report.get("source_candidate_id"),
        report.get("sprint_status"),
        report.get("selected_rescue_decision"),
        report.get("passed_all_train_validation_gates"),
        report.get("candidate_promotable_to_oos_review"),
        report.get("candidate_closed"),
        report.get("recommended_next_step"),
        safety_locked,
    ]
